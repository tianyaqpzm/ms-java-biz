...
public class PythonAgentClientImpl implements PythonAgentClient {
    ...
    public PythonAgentClientImpl(WebClient webClient) {
        this.webClient = webClient;
    }

    @Override
    public IngestDocumentResponse ingestDocument(IngestDocumentRequest request) {
        String pythonUrl = "http://" + RemoteApiConstants.PythonAgent.SERVICE_NAME + RemoteApiConstants.PythonAgent.KNOWLEDGE_DOCUMENT_INGEST;
        
        // 将领域层 DTO 转换为生成的 OpenAPI 实体
        IngestRequest remoteRequest = new IngestRequest()
                ...
                .topK(request.topK())
                .scoreThreshold(request.scoreThreshold() != null ? request.scoreThreshold().floatValue() : null)
                .enableHybridSearch(request.enableHybridSearch())
                .alphaWeight(request.alphaWeight() != null ? request.alphaWeight().floatValue() : null)
                .generationModel(request.generationModel())
                .temperature(request.temperature() != null ? request.temperature().floatValue() : null)
        ...
        try {
            return webClient.post()
                    .uri(pythonUrl)
                    .bodyValue(remoteRequest)
                    .retrieve()
                    .onStatus(HttpStatusCode::isError, response -> 
                        response.bodyToMono(RemoteErrorResponse.class)
                                .flatMap(errorBody -> {
                                    log.error("ms-py-agent returned error: status={}, body={}", response.statusCode(), errorBody);
                                    String errorCode = errorBody.error_code() != null ? errorBody.error_code() : String.valueOf(response.statusCode().value());
                                    String errorMsg = errorBody.error_msg() != null ? errorBody.error_msg() : "Remote call failed";
                                    return Mono.error(new BusinessException(errorCode, errorMsg));
                                })
                                .cast(Throwable.class)
                                .switchIfEmpty(Mono.error(new BusinessException(String.valueOf(response.statusCode().value()), "Remote call failed with no body")))
                    )
                    .bodyToMono(IngestDocumentResponse.class)
                    .block();
        } catch (Exception e) {
            log.error("Failed to call ms-py-agent via WebClient", e);
